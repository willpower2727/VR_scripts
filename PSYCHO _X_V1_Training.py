...
This is the active training task for the passive psychometric testing.

A subject stands in a neutral position with feet placed together. On que, subject takes two steps forward with X LENGTH

Biofeedback is shown as a grid display as in Vazquez et al 2015. Subjects are shown their HIP-ANK distance @ HS
...
targetmean = (targetXr+targetXl)/2
...
#	timeold = time.time()
	...
	while not endflag.isSet():
		...
		if root is None:
			...
			#boxR.setScale(0.2,Ralpha,0) --> ? why doesn't this match what was happening before?
			boxR.setScale(0.2,Rx,0)
			boxL.setScale(0.2,Lx,0)

			#adjust alpha on right leg
			if (phaxxe == 0):
				messagewin.message(str(phaxxe))
				if (Rz < -30) & (Lz < -30) & (abs(targetUr-Ralpha) >= 0.04):
					Rspeed = int(300*math.copysign(1,-1*(targetUr-Ralpha)))
					Lspeed = 0
				elif (Rz < -30) & (Lz < -30) &(abs(targetUr-Ralpha) < 0.04):
					Rspeed = 0
					Lspeed = 0
					phaxxe = 1
				else:
...
def savedata(savestring,q3):
	...
	mst = time.time()
	mst2 = int(round(mst))
	mststring = str(mst2)+'SLBF_V4_Fam_rev1_DK2.txt'
