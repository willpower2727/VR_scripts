...
#
#Tested 10/26/2015 @ 100Hz
#
#Rev 3 allows user to easily setup targets by loading the target values froma file created by a Matlab GUI "EffortStudyTargets.m"
#
#Use V2P_DK2_R1
...
Eyedistance = -1
viz.MainView.setPosition(0,0,Eyedistance)
...
global boxL
boxL = viz.addTexQuad(pos=[-0.2-wideLX,0,Eyedistance+distLZ],scale=[0.2+wideLX,2*targettol*lscale,0])
boxL.color(0,0.5,1)
boxL.alpha(0.5)

global boxR
boxR = viz.addTexQuad(pos=[0.2+wideRX,0,Eyedistance+distRZ],scale=[0.2+wideRX,2*targettol*rscale,0])
boxR.color(0,0.5,1)
boxR.alpha(0.5)
#boxR.visible(0)

global HistBallR
HistBallR = viz.add('box.wrl', color=(2,2,0.7), scale=[0.2+wideRX,0.01*rscale,0.001], cache=viz.CACHE_NONE)
HistBallR.setPosition([0.2+wideRX,0,Eyedistance+distRZ])
HistBallR.alpha(0.8)
#HistBallR.visible(0)

global HistBallL
HistBallL = viz.add('box.wrl', color=(2,2,0.7), scale=[0.2+wideLX,0.01*lscale,0.001], cache=viz.CACHE_NONE)
HistBallL.setPosition([-0.2-wideLX,0,Eyedistance+distLZ])
HistBallL.alpha(0.8)
#HistBallL.visible(0)

global cursorR
#cursorR = viz.add('box3.obj', color=viz.RED, scale=[0.1,0.25,0.001], cache=viz.CACHE_NONE)
cursorR = viz.add('box3.obj', color=viz.RED, scale=[0.2+wideRX,0.01*rscale,0.001], cache=viz.CACHE_NONE)
#cursorR.setPosition([0.2,-1*faketarget,0])
cursorR.setPosition([0.2+wideRX,-1*faketarget,Eyedistance+distRZ])
...
cursorL = viz.add('box3.obj', color=viz.GREEN, scale=[0.175+wideLX,0.01*lscale,0.001], cache=viz.CACHE_NONE)
#cursorL.setPosition([-0.2,-1*faketarget,0])
cursorL.setPosition([-0.2-wideLX,-1*faketarget,Eyedistance+distLZ])
...
#		print('RANKY: ',RANKY, ' LANKY: ',LANKY)
		...
		cursorR.setScale(0.19+wideRX,(LANKY-RANKY)*rscale,0.001)
		cursorL.setScale(0.19+wideLX,(RANKY-LANKY)*lscale,0.001)
		#RHS
		if (Rforceold >= -30) & (Rz < -30):
			steplengthR = LANKY-RANKY
			print('R error',abs(steplengthR-targetR))
			HistBallR.setPosition([0.2+wideRX,-0.25+steplengthR*rscale,Eyedistance+distRZ])
#			HistBallR.setPosition([0.2,-0.25+steplengthR*rscale,0])
		...
		if (Lforceold >= -30) & (Lz < -30):
			...
			HistBallL.setPosition([-0.2-wideLX,-0.25+steplengthL*lscale,Eyedistance+distLZ])
			LHS = 1
...
def savedata(savestring,q3):
	...
	mst = time.time()
	mst2 = int(round(mst))
	mststring = str(mst2)+'EffortStudy_static_R3.txt'
