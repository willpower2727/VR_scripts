...
#				if (abs(LTOEX-RTOEX+xoff)<latt/2) & (abs(LTOEY-RTOEY+yoff)<sagt/2):
#					cursor.visible(1)
#				else:
#					cursor.visible(0)
...
#				if (abs(RTOEX-LTOEX-xoff)<latt/2) & (abs(RTOEY-LTOEY-yoff)<sagt/2):
#					cursor.visible(1)
#				else:
#					cursor.visible(0)
			...
			if (LEG==0):
				cursor.setPosition(transcale*(LTOEX-RTOEX+xoff),transcale*(LTOEY-RTOEY+yoff),-0.02)
				#look for touch
				if (Rz <=-30) & (touchdown > 50): #touch must last 0.5 seconds
					RHS = 1
		...
		histzL = Lz
		#save data
		savestring = [FN,Rz,Lz,RHS,LHS,rtouchx,ltouchx,rtouchy,ltouchy,phase,highlight[stepind],RTOEX,LTOEX,RTOEY,LTOEY]#organize the data to be written to file
		q3.put(savestring)
...
def savedata(savestring,q3):
	...
	csvw.writerow(['FrameNumber','Rfz','Lfz','RHS','LHS','rtouchx','ltouchx','rtouchy','ltouchy','phase'])
