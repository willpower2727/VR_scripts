...
#V2P DK2 R2
#
#Rev 5 uses DK2 markers to track head position and segment data to track orientation, different from version 3 in that it looks at rotation matrix directly
#
#WDA 3/2/2016
...
#		try:
		...
		savestring = [FN,Rz,Lz,np.mean(alphastack),np.mean(betastack),np.mean(gammastack),np.mean(xstack),np.mean(ystack),np.mean(zstack)]
		q3.put(savestring)
...
def savedata(savestring,q3):
	...
	mst = time.time()
	mst2 = int(round(mst))
	mststring = str(mst2)+'ScrollingSteps_rev5.txt'
	print("Data file created named: ")
	print(mststring)
	file = open(mststring,'w+')
#	json.dump(['FrameNumber','Rfz','Lfz','RHIPy','LHIPy','RANky','LANKy','fastSLASYM','slowSLASYM','RHS','LHS'],file)
	json.dump(['FrameNumber','Rfz','Lfz','alpha','beta','gamma','hmdx','hmdy','hmdz'],file)
	file.close()
