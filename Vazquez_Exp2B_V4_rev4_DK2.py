#Exp2B V4 alteration of "Perceptual Sensory Correlates of Split Belt Walking Adaptation"
#2 steps
#
...
#
#V4 is an evaluation where subjects will have no feedback on right side
#
#rev4 changes the display to 3D, with stepping stone targets
#
#V2P_DK2_R2
#
#William Anderton 3/17/2016
...
global messagewin
messagewin = viz.addText('MOVING',pos=[0.3,1.5,0],scale=[0.1,0.1,0.1])
...
#look at where things are ligning up
import VizardOrigin
...
#targets
global boxR
boxR = vizshape.addBox(size=[0.25,0.01,2*targettol])
boxR.setPosition(0.462+0.2,0.011,-1*(1.45-targetR))
boxR.color(0,0,0)
global boxL
boxL = vizshape.addBox(size=[0.25,0.01,2*targettol])
...
#		print("RANKY",RANKY)
		...
		gammastack.pop(0)
		viewLink.setOffset([-1*np.mean(alphastack),np.mean(gammastack),-1*np.mean(betastack)])#two meters behind
...
def savedata(savestring,q3):
	...
	mst = time.time()
	mst2 = int(round(mst))
	mststring = str(mst2)+'EXP2B_DK2_V4_rev4.txt'
