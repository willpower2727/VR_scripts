""" Step Length Biofeedback familiarization task, Version 2, uses DK2

A subject stands in a pre-step pose. On que, subject takes a step forward towards a target distance.
...
#				viz.visible(0)#turn off display
				highlightr.color(viz.BLUE)
				highlightl.color(viz.BLUE)
				try:
					if (randy[stepind] == 1):#right leg next test
...
#					leftcounter.message(str(LCOUNT)+'/'+str(Lattempts))
			...
			histzL = Lz
			#save data
			savestring = [FN,Rz,Lz,RHS,LHS,rgorb,lgorb,RANKY-LANKY,LANKY-RANKY,targetUr-(LANKY-RANKY),targetUl-(RANKY-LANKY),RANKY,LANKY]#organize the data to be written to file
			q3.put(savestring)
#			timeold = time.time()
	cpps.kill()
#	q3.join()
...
def savedata(savestring,q3):
	...
	mst = time.time()
	mst2 = int(round(mst))
	mststring = str(mst2)+'SLBF_V2_Fam_rev1_DK2.txt'
