...
#					targetR = abs(Rhsp)+(1/(1+R))*(abs(Rtop)-(R*abs(Rhsp)))
					...
					print(targetR)
					psudoR = 1
					boxR.setPosition([0.2,targetR*scaling,0])#targetR doesn't change immediately with catchflag on
			else:
				RHS = 0
				RTO = 0
