...
global boxL
boxL = viz.addChild('target.obj',color=(0.063,0.102,0.898),scale=[0.2,targettol,0.0125])
boxL.setPosition([-0.3,targetL-targettol,0])
#boxL.alpha(0.6)

global boxR
boxR = viz.addChild('target.obj',color=(0.063,0.102,0.898),scale=[0.2,targettol,0.0125])
boxR.setPosition([0.3,targetR-targettol,0])
#boxR.alpha(0.6)

viz.MainView.setPosition(0,0.85,-2.5)
viz.MainView.setEuler(0,0,0)
...
global cursorR
cursorR = viz.add('box2.obj', color=viz.GRAY, scale=[0.1,0.05,0.0125], cache=viz.CACHE_NONE)
cursorR.setPosition([0.3,0,0])

global cursorL
cursorL = viz.add('box2.obj', color=viz.GRAY, scale=[0.1,0.05,0.0125], cache=viz.CACHE_NONE)
cursorL.setPosition([-0.3,0,0])

global HistBallR
HistBallR = viz.add('box.wrl', color=viz.YELLOW, scale=[0.2,0.05,0.0125], cache=viz.CACHE_NONE)
HistBallR.setPosition([0.3,targetR,0])
HistBallR.alpha(0.8)
...
global HistBallL
HistBallL = viz.add('box.wrl', color=viz.YELLOW, scale=[0.2,0.05,0.0125], cache=viz.CACHE_NONE)
HistBallL.setPosition([-0.3,targetL,0])
HistBallL.alpha(0.8)
...
def onVectorValueReceived(vectorName, vectorValue, invalid):
	...
	if (vectorName == 'BallPositionR') and not invalid:                     # look for the BallPosition variable in the data stream#
		...
		cursorR.setScale(0.1,vectorValue[2],0.01250)#move ball according to step time in Z direction
		
		if (vectorValue[2] <= 0.1):
			HistBallR.setPosition([0, histR, 0])#update yellow history ball when HS happens
			steptimeR = histR
		else:
			histR = vectorValue[2]
			
		if (abs(steptimeR-targetR) <= targettol-0.041):#highlight the target when the target is hit
	...
	if (vectorName == 'BallPositionL') and not invalid:  
		...
		cursorL.setPosition(- vectorValue[1], vectorValue[2], vectorValue[0]) 
		if (vectorValue[2] <= 0.1):
			HistBallL.setPosition([-0.3, histL, 0])
			steptimeL = histL
		else:
			histL = vectorValue[2]
		
		if (abs(steptimeL-targetL) <= targettol-0.041):
