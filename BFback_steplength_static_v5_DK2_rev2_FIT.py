...
V5 uses a compound method to test how subjects step. Initially, both feet are together in the Y direction. step length is tested in a compound manner meaning that
...
#						HistBallL.setPosition([-0.2,RANKY-LANKY+1.45-RANKY, 0])
						if (abs((RSL+LSL)-(targetUl+targetUr)) <= targettol):
							LCOUNT = LCOUNT+1
#							boxL.color( viz.WHITE )
							lgorb = 1
						else:
...
def savedata(savestring,q3):
	...
	mst = time.time()
	mst2 = int(round(mst))
	mststring = str(mst2)+'rev2DK2V5.txt'
