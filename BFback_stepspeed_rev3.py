...
def onVectorValueReceived(vectorName, vectorValue, invalid):
	...
	if (vectorName == 'BallPositionR') and not invalid:                     # look for the BallPosition variable in the data stream#
		...
		cursorL.setScale(0.1,vectorValue[2],0.01250)#scale ball according to step time in Z direction
		
		if (vectorValue[2] <= 0.1):
			HistBallL.setPosition([-0.2, histR, 0])#update yellow history bar when HS happens
			steptimeR = histR
		else:
			histR = vectorValue[2]
			'''
	...
	if (vectorName == 'BallPositionL') and not invalid:  
		...
		cursorR.setScale(0.1,vectorValue[2],0.01250)#scale cursor according to step time in Z direction
		
		if (vectorValue[2] <= 0.1):
			HistBallR.setPosition([0.2, histL, 0])
			steptimeL = histL
		else:
			histL = vectorValue[2]
		'''
