...
#					Rspeed = 0
#					Lspeed = 0
			elif (phaxxe == 2):  #We wait to get a readinf from the readmill indicating that the belts have come to a stop
				#time.sleep(2)
				messagewin.message(str(phaxxe))
...
#					print('Length q4: ', q4.qsize() ,' RBS: ',SpdPak[1], 'LBS: ',SpdPak[2])
			...
			if ((stepind)<(maxstep)):
				...
				savestring = [FN,Rz,Lz,RHS,LHS,RANKY-LANKY,LANKY-RANKY,RANKY,LANKY,PSYCHO, frodo[stepind], time.time(), stepind, step, phaxxe, desiredSL, targetSL, plateau, AVGtargetSL, reftime]#organize the data to be written to file			
			print('$$$$$$$$ Stepind :', stepind, ' MAX STEP: ', maxstep)
			q3.put(savestring)
...
def savedata(savestring,q3):
	...
	csvw.writerow(['FrameNumber','Rfz','Lfz','RHS','LHS','rgamma','lgamma','RANK','LANK', 'PSYCHO', 'frodo', 'time', 'stepind', 'step', 'phaxxe', 'desiredSL', 'targetSL','plateau', 'AVGtargetSL', 'reftime'])
	file.close()
