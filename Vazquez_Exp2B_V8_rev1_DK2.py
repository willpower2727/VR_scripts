...
#
#V8 displays no feedback but is otherwise identical to V7
#
#V2P_DK2_R1
#
#William Anderton 4/7/2016
...
for x in range(1,23,1):
	...
	lines["T{0}".format(x)].setPosition(0,0.2+(x-1)*0.02,0)
#	print((x-1)*0.02)
global tnums
tnums = {}
for x in range(0,21,1):
	if (x<10):
		tnums["Num{0}".format(x)]=viz.addText(str(x),pos=[0,0.2+x*0.02+0.005,0],scale=[0.015,0.015,0.015])
	else:
		tnums["Num{0}".format(x)]=viz.addText(str(x),pos=[-0.005,0.2+x*0.02+0.005,0],scale=[0.015,0.015,0.015])
	
viz.MainView.setPosition(0, 0.49, -0.57)
...
#		Lalpha = (LHIPY+RHIPY)/2-LANKY
		...
		histzL = Lz
		#save data
		savestring = [FN,Rz,Lz,RHS,LHS,RANKY,LANKY,rgorb,lgorb,Rgamma,Lgamma,Rerror,Lerror,targetnum,]#organize the data to be written to file
		q3.put(savestring)
...
def savedata(savestring,q3):
	...
	mst = time.time()
	mst2 = int(round(mst))
	mststring = str(mst2)+'EXP2B_V8_rev1_DK2.txt'
	print("Data file created named: ")
	print(mststring)
	file = open(mststring,'w+')
	json.dump(['FrameNumber','Rfz','Lfz','RHS','LHS','RANKY','LANKY','RGORB','LGORB','Rgamma','Lgamma','Rerror','Lerror','targetnumber'],file)
	file.close()
