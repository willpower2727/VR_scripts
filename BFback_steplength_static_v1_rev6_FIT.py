...
rev 6 introduces the python c++ server app instead of TMM
...
#				HistBallL.visible(0)
			...
			if (Rz <= -30) & (LANKY-RANKY > 0.05):#check to see if the treadmill needs to move the feet back to neutral
				speedlist = [200,0,1000,1000,0]
				qq.put(speedlist)
			elif (Lz <= -30) & (RANKY-LANKY > 0.05):
			...
			if (abs(LANKY-RANKY) <= 0.05):#feet are together
				try:
					if (randy[stepind]  == 1):#when the feet are back together, display the next target
						boxR.visible(1)
						boxL.visible(0)
					else:
						boxR.visible(0)
