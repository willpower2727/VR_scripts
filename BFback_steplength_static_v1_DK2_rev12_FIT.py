...
In version 1 rev 12 everything is identical to a training round, except the cursors are off and the step length feedback bar displays a fake value from a distribution of errors.
so the subject does not have true feedback.
...
#			LHIPY = float(tempdat[6])/1000
#			RANKY = float(tempdat[5])/1000
...
def savedata(savestring,q3):
	...
	mst = time.time()
	mst2 = int(round(mst))
	mststring = str(mst2)+'rev12V1.txt'
