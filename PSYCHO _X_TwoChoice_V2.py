...
ranTar=[0, .04, -.04, .08, -.08, .12, -.12, .20, -.20] #CJS 1/16/2016
sets=1
...
#							plateau=0
			...
			if (phaxxe == 0):
				...
				if (Rz < -30) & (Lz < -30) & (abs(targetSL-desiredSL) >= 0.04)  : #Undershot the target outside the tolerance of 0.04
				...
				elif (Rz < -30) & (Lz < -30) &(abs(targetSL-desiredSL) < 0.04): #Within the target tolerance of 0.04
					Rspeed = 0
					Lspeed = 0
					phaxxe = 2
				else:
				...
				elif (Rz < -30) & (Lz < -30) &(abs(targetSL-desiredSL) < 0.01): #Within the target tolerance of 0.04
					Rspeed = 0
					Lspeed = 0
					phaxxe = 3
				else:
			...
			elif (phaxxe == 3): #Only move one belt so that we can be more accuate
				messagewin.message(str(phaxxe))
				if (Rz < -30) & (Lz < -30) & (abs(targetSL-desiredSL) >= 0.005)  : #Undershot the target outside the tolerance of 0.04
					#print ("A. AlphaDiff: ", (targetALPHA-desiredALPHA), " XDiff: ", (targetXXX-desiredX), " RightFirst: ", RIGHT)
					if RIGHT==0:
						Lspeed = int(20*math.copysign(1,-1*(targetSL-desiredSL)))
					elif RIGHT==1:
						Rspeed = int(20*math.copysign(1,-1*(targetSL-desiredSL)))
				elif (Rz < -30) & (Lz < -30) &(abs(targetSL-desiredSL) < 0.005): #Within the target tolerance of 0.01
					Rspeed = 0
					Lspeed = 0
					reftime = time.time()
					phaxxe = 5
			...
			histzL = Lz
			#save data
#			savestring = [FN,Rz,Lz,RHS,LHS,rgorb,lgorb,RANKY-LANKY,LANKY-RANKY,Ralpha,Lalpha,Rx,Lx,RANKY,LANKY,RGT,LGT,phaxxe, PSYCHO, targetAmean, targetXmean, frodo[stepind], time.time(), stepind, step, phaxxe, desiredSL, targetSL, plateau, AVGtargetSL, reftime  ]#organize the data to be written to file			
			savestring = [FN,Rz,Lz,RHS,LHS,rgorb,lgorb,RANKY-LANKY,LANKY-RANKY,RANKY,LANKY,phaxxe, PSYCHO, frodo[stepind], time.time(), stepind, step, phaxxe, desiredSL, targetSL, plateau, AVGtargetSL, reftime  ]#organize the data to be written to file			
			q3.put(savestring)
#			timeold = time.time()
	cpps.kill()
#	q3.join()
...
#	savestring = [FN,Rz,Lz,RHS,LHS,rgorb,lgorb,RANKY-LANKY,LANKY-RANKY,Ralpha,Lalpha,Rx,Lx,RANKY,LANKY,RGT,LGT,phaxxe, PSYCHO, targetAmean, targetXmean, frodo, time]
	csvw.writerow(['FrameNumber','Rfz','Lfz','RHS','LHS','rgorb','lgorb','rgamma','lgamma','Ralpha','Lalpha','Rx','Lx','RANK','LANK','RHIP','LHIP','phase', 'PSYCHO', 'targetAmean', 'targetXmean', 'frodo', 'time', 'stepind', 'step', 'phaxxe', 'desiredSL', 'targetSL','plateau', 'AVGtargetSL', 'reftime'])
	file.close()
