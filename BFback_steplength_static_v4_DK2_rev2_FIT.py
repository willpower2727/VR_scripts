...
In version 4 feedback, cursor, and target are on. this is for training
rev2 incorporates new methods for streaming data, as well as using the DK2 for display 
...
#						cursorL.setPosition([-0.2,0,0.025])
						if (LANKY-RANKY < 0.04):
							...
							cursorR.visible(1)
							cursorL.visible(0)
							cursorR.setScale(0.1,LSL+LANKY-RANKY,0.01250)
#						cursorL.visible(0)
					elif (randy[stepind] == 2):
...
#							cursorL.setScale(0.1,targetXl+RANKY-LANKY,0.01250)
							cursorL.setScale(0.1,RSL+RANKY-LANKY,0.01250)
#						cursorR.visible(0)
				except:
					pass
...
#					cursorL.visible(1)
					if (randy[stepind] == 1):#right leg test
						HistBallR.setPosition([0.2,LSL+RSL, 0])
						if (abs((RSL+LSL)-(targetUr+targetUl)) <= targettol):
							RCOUNT = RCOUNT+1
...
#						boxL.setPosition([-0.2,targetUl+1.45-RANKY,0])
						HistBallR.setPosition([0.2,RSL, 0])
...
#						boxR.setPosition([0.2,1.45-LANKY+targetUr,0])
						HistBallL.setPosition([-0.2,LSL, 0])
						if (abs(LSL-targetXl) <= targettol):
							LCOUNT = LCOUNT+1
						...
						HistBallL.setPosition([-0.2,RSL+LSL, 0])
						if (abs((LSL+RSL)-(targetUl+targetUr)) <= targettol):
							LCOUNT = LCOUNT+1
							boxL.color( viz.WHITE )
							lgorb = 1
						else:
			...
			#save data
			try:
				savestring = [FN,Rz,Lz,rgorb,lgorb,RANKY,LANKY,targetXr-(LANKY-RANKY),targetXl-(RANKY-LANKY),(targetUr+targetUr)-(LANKY-RANKY),(targetUl+targetUr)-(RANKY-LANKY),randy[stepind]]#organize the data to be written to file
			except:
...
def savedata(savestring,q3):
	...
	mst = time.time()
	mst2 = int(round(mst))
	mststring = str(mst2)+'rev2DK2V4.txt'
	print("Data file created named: ")
	print(mststring)
	file = open(mststring,'w+')
	json.dump(['FrameNumber','Rfz','Lfz','RGORB','LGORB','RANK','LANK','Xrerror','XLerror','Urerror','Ulerror','test'],file)
	file.close()
