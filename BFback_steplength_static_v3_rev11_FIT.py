...
rev11 is identicle to rev10 except it is now saving some data
wda 2/5/2015
"""
import viz
import vizshape
...
#			ln = root.find(".//LatencyTotal")#find the frame number
#			lnn = ln.attrib.values()
			...
			if (Rz <= -30) & (histzR > -30):#RHS condition
				...
				if (abs((LANKY-RANKY)-targetR) <= targettol):
					...
					boxR.color( viz.BLUE )
					rgorb = 0
				rightcounter.message(str(RCOUNT)+'/'+str(Rattempts-1))
			...
			if (Lz <= -30) & (histzL > -30):#LHS condition
				...
				if (abs((RANKY-LANKY)-targetL) <= targettol):
					...
					boxL.color( viz.BLUE )
					lgorb = 0
				leftcounter.message(str(LCOUNT)+'/'+str(Lattempts-1))
			...
			if (Rz <= -30) & (LANKY-RANKY > 0.04):#check to see if the treadmill needs to move the feet back to neutral
				speedlist = [200,0,1000,1000,0]
				qq.put(speedlist)
			elif (Lz <= -30) & (RANKY-LANKY > 0.04):
				speedlist = [0,200,1000,1000,0]
				qq.put(speedlist)
			else:
				speedlist = [0,0,1000,1000,0]
				qq.put(speedlist)
			if (abs(LANKY-RANKY) <= 0.05):#feet are together
				try:
