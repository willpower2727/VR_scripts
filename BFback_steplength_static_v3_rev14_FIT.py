...
rev14 only moves one leg at a time and seeks to instruct the subject 
uses the compound step length method
			...
			if (phaxxe == 0):  #match ankles if needed
				try:
					if (randy[stepind-1] == 1) & (randy[stepind] == 1):#previously a right step next is also right
						...
						if (Rz < -30) & (Lz < -30) &(abs(1.45-targetXl-RANKY) >= 0.04):
							Rspeed = int(300*math.copysign(1,1.45-targetXl-RANKY))
						elif (Rz < -30) & (Lz < -30) &(abs(1.45-targetXl-RANKY) < 0.04):
							Rspeed = 0
							phaxxe = 1
						else:
							Rspeed = 0
					elif (randy[stepind-1] == 2) & (randy[stepind] == 1): #need to match ankles first
						if (Rz < -30) & (Lz < -30) & (abs(1.45-targetXr-LANKY) >= 0.04):
							Lspeed = int(300*math.copysign(1,1.45-targetXr-LANKY))
						elif (Rz < -30) & (Lz < -30) & (abs(1.45-targetXr-LANKY) < 0.04):
							Lspeed = 0
							phaxxe = 1
						else:
							Lspeed = 0
