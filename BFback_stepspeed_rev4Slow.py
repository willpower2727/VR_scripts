...
def UpdateViz(root,q):
	...
	while not endflag.isSet():
		...
		temp4 = float(temp3[0])
		cursorR.setScale(0.1,0.8*lstancetime,0.01250)
		cursorL.setScale(-0.1,0.8*rstancetime,0.01250)
		...
		if (temp2 <= -30) & (histzR > -30):#Right HS condition
			...
			HistBallL.setPosition([-0.2,0.8*rstancetime, 0])
			rstancetime = 0
		...
		if (temp4 <= -30) & (histzL > -30):#Right HS condition
			...
			HistBallR.setPosition([0.2, 0.8*lstancetime, 0])
			lstancetime = 0
