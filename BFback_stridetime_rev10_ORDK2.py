#Stridetime rev10 is the first revision to use Occulus DK2, allows head movements
#wda 9/25/2015
#V2P_R1
#view is locked to prevent sickness???
...
#		print(Queue.Queue.qsize(q))
		...
		p1x = root.find(".//Subject0/HMD1/X")
		p1y= root.find(".//Subject0/HMD1/Y")
		p1z = root.find(".//Subject0/HMD1/Z")
		
		p2x = root.find(".//Subject0/HMD2/X")
		p2y= root.find(".//Subject0/HMD2/Y")
		p2z = root.find(".//Subject0/HMD2/Z")
		
		p3x = root.find(".//Subject0/HMD3/X")
		p3y= root.find(".//Subject0/HMD3/Y")
		p3z = root.find(".//Subject0/HMD3/Z")
...
#		viz.MainView.setPosition(-1*pc1x,pc1z,-1*pc1y)
		...
		if (Rz < -30) & (histzR >= -30):#HS condition
			HistBallR.setPosition([0.2, rstridetime*scalorxx, 0.5])
			RHS = 1
		...
		if (Lz < -30) & (histzL >= -30):
			HistBallL.setPosition([-0.2, lstridetime*scalorxx, 0.5])
			LHS = 1
...
def savedata(savestring,q3):
	...
	mst = time.time()
	mst2 = int(round(mst))
	mststring = str(mst2)+'StrideTime_R10.txt'
