...
global targetL
targetL = 0.7180 #THIS IS THE HEIGHT OF THE TARGET

global targetR
targetR = 0.7180

global targettol
targettol = 0.025 #THIS IS THE HEIGHT OF THE COLORED BOXES

global boxL #THIS IS THE PLOTTING CODE FOR THE BOXES
...
global cursorR #THESE ARE THE GREY BOXES
...
global HistBallR #THESE ARE THE YELLOW LINES, representing the value from the previous step
...
def UpdateViz(root,q,savestring,q3):
	...
	while not endflag.isSet():
		root = q.get()
		timediff = time.time()-timeold #determine how much time has passed since the privious iteration of the loop, this information is used to comupute step time.
		...
		if (Rz < -30) & (histzR >= -30):#RHS: The FIRST time the right force exceeds 30 N
			cursorR.visible(0)# hide right side show left, right foot contact, so this is the start of a left step
			cursorL.visible(1)
			RHS = 1
			targetL=targetL-0.01; boxL.setPosition([-0.2,targetL,0]) #Tryout Digna do gradually decrease left step length 
			if (abs(steptime - targetR) < targettol): #the RIGHT step has now completed, so this code evauates the step. 
				boxR.color(0,1,0.3)
				rgorb = 1
			else:
				boxR.color(1,0.2,0)
			...
			steptime = 0 # AS this if loop defines the beginning of a step, the step time should be set at zero.
		
		elif (Lz < -30) & (histzL >= -30): #LHS
			cursorR.visible(1)
			cursorL.visible(0)
			LHS = 1
			targetR=targetR+0.01;boxR.setPosition([0.2,targetR,0]) #Tryout Digna do gradually increase right step length 
			...
			RHS = 0
			LHS = 0
			steptime = steptime + timediff #increase step time untill it is set to zero again (i.e. at instant of new step detection)
		#change the heights
...
def savedata(savestring,q3):
	...
	mst = time.time()
	mst2 = int(round(mst))
	mststring = str(mst2)+'BFback_steptime_rev7TEST_Digna.txt'
