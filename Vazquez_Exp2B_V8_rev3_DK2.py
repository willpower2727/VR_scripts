...
#
#Subjects walk at a set speed, for 30 strides they try to match step positions (ANK-ANK distance) 
#targets are displayed on a latitudinal grid, 20 targets, 2 cm apart.
#
#The target is determined based on TM baseline walking and remain fixed throughout the whole trial (originally fast leg was to match slow leg step position) 
#
#V8 is atest version of V7, no feedback is displayed
#REV 3 uses csv write method instead of JSON, improves post processing time
#
#V2P_DK2_R1
#
...
#	print(randy)
	frodo = frodo+[randy[0]]*25
	frodo = frodo+[randy[1]]*25
	frodo = frodo+[randy[2]]*25
...
#				pass
		...
		if (Rz <= -30) & (histzR > -30):#RHS
			Rerror = targetR+frodo[stepind]-Rgamma
			RHS = 1
...
#			highlightr.setPosition(0.145,targetmean+frodo(stepind),0)
		...
		if (Lz <= -30) & (histzL > -30):#LHS
			Lerror = targetL+frodo[stepind]-Lgamma
			LHS = 1
...
#			targetnum = updatetargetdisplay(Lgamma,circleL,1)
		...
		histzL = Lz
		#save data
		savestring = [FN,Rz,Lz,RHS,LHS,RANKY,LANKY,rgorb,lgorb,Rgamma,Lgamma,Rerror,Lerror,targetmean+frodo[stepind],]#organize the data to be written to file
		q3.put(savestring)
...
def savedata(savestring,q3):
	...
	mst = time.time()
	mst2 = int(round(mst))
	mststring = str(mst2)+'EXP2B_V8_rev3_DK2.txt'
	print("Data file created named: ")
	print(mststring)
	file = open(mststring,'w+')
	csvw = csv.writer(file)
	csvw.writerow(['FrameNumber','Rfz','Lfz','RHS','LHS','RANKY','LANKY','RGORB','LGORB','Rgamma','Lgamma','Rerror','Lerror','target'])
#	json.dump(['FrameNumber','Rfz','Lfz','RHS','LHS','RANKY','LANKY','RGORB','LGORB','Rgamma','Lgamma','Rerror','Lerror','target'],file)
	file.close()
