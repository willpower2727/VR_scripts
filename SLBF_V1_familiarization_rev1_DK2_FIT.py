""" Step Length Biofeedback familiarization task, Version 1, uses DK2

A subject stands in a neutral position with feet placed together. On que, subject takes a step forward towards a target distance.
...
#					leftcounter.message(str(LCOUNT)+'/'+str(Lattempts))
			...
			histzL = Lz
			#save data
			savestring = [FN,Rz,Lz,rgorb,lgorb,RANKY-LANKY,LANKY-RANKY,targetUr-(LANKY-RANKY),targetUl-(RANKY-LANKY),RANKY,LANKY]#organize the data to be written to file
			q3.put(savestring)
#			timeold = time.time()
	cpps.kill()
#	q3.join()
...
def savedata(savestring,q3):
	...
	mst = time.time()
	mst2 = int(round(mst))
	mststring = str(mst2)+'SLBF_V1_Fam_rev1_DK2.txt'
	print("Data file created named: ")
	print(mststring)
	file = open(mststring,'w+')
	csvw = csv.writer(file)
	csvw.writerow(['FrameNumber','Rfz','Lfz','rgorb','lgorb','rgamma','lgamma','rerror','lerror','RANK','LANK'])
	file.close()
